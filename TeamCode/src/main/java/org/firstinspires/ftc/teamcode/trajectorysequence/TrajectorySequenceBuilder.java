...
public class TrajectorySequenceBuilder {
	...
	private TrajectorySequenceBuilder addPath(AddPathCallback callback) {
		...
		double displacementDifference =
			builtTraj.getPath().length() - lastDisplacementTraj;
	...
	public TrajectorySequenceBuilder setReversed(boolean reversed) {
		return reversed
			? this.setTangentOffset(Math.toRadians(180.0))
	...
	public TrajectorySequenceBuilder setVelConstraint(
		TrajectoryVelocityConstraint velConstraint
	) {
		this.currentVelConstraint = velConstraint;
	...
	public TrajectorySequenceBuilder setTurnConstraint(
		double maxAngVel,
		double maxAngAccel
	) {
	...
	public TrajectorySequenceBuilder addSpatialMarker(
		Vector2d point,
		MarkerCallback callback
	) {
	...
	public TrajectorySequenceBuilder UNSTABLE_addDisplacementMarkerOffset(
		...
		return this.addDisplacementMarker(
	...
	public TrajectorySequenceBuilder addDisplacementMarker(
		...
		return addDisplacementMarker(
			(displacement -> scale * displacement + offset),
			callback
		);
	}
	...
	public TrajectorySequenceBuilder turn(double angle) {
		return turn(
			...
			currentTurnConstraintMaxAngAccel
		);
	}

	public TrajectorySequenceBuilder turn(
		double angle,
		double maxAngVel,
		double maxAngAccel
	) {
	...
	public TrajectorySequenceBuilder waitSeconds(double seconds) {
		pushPath();
		sequenceSegments.add(
			new WaitSegment(lastPose, seconds, Collections.emptyList())
		);
	...
	private List<TrajectoryMarker> convertMarkersToGlobal(
		...
		for (DisplacementMarker marker : displacementMarkers) {
			...
			trajectoryMarkers.add(
				new TrajectoryMarker(time, marker.getCallback())
			);
		}
	...
	private List<SequenceSegment> projectGlobalMarkersToLocalSegments(
		...
		for (TrajectoryMarker marker : markers) {
			...
			if (segment instanceof WaitSegment) {
				...
				newMarkers.addAll(
					sequenceSegments.get(segmentIndex).getMarkers()
				);
				newMarkers.add(
					new TrajectoryMarker(
						segmentOffsetTime,
	...
	private Double motionProfileDisplacementToTime(
	...
	private Double displacementToTime(
		List<SequenceSegment> sequenceSegments,
		double s
	) {
		...
		for (SequenceSegment segment : sequenceSegments) {
			if (segment instanceof TrajectorySegment) {
				...
				double segmentLength = thisSegment
					.getTrajectory()
					.getPath()
					.length();
	...
	private Double pointToTime(
		...
		for (SequenceSegment segment : sequenceSegments) {
			if (segment instanceof TrajectorySegment) {
				...
				double displacement = thisSegment
					.getTrajectory()
					.getPath()
					.project(point, 0.25);
		...
		for (ComparingPoints comparingPoint : projectedPoints) {
			...
			if (
				comparingPoint.distanceToPoint < closestPoint.distanceToPoint
			) closestPoint = comparingPoint;
		}

		return displacementToTime(
			sequenceSegments,
			closestPoint.thisPathDisplacement
		);
	}
