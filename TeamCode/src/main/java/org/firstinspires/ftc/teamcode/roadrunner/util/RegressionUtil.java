...
public class RegressionUtil {
	 ...
	 */
	private static List<Double> numericalDerivative(
		List<Double> x,
		List<Double> y
	) {
		...
		for (int i = 1; i < x.size() - 1; i++) {
			deriv.add(
				(y.get(i + 1) - y.get(i - 1)) / (x.get(i + 1) - x.get(i - 1))
			);
		}
	...
	public static RampResult fitRampData(
		...
		List<Double> velSamples = numericalDerivative(
			timeSamples,
			positionSamples
		);
	...
	public static AccelResult fitAccelData(
		...
		List<Double> accelSamples = numericalDerivative(
			timeSamples,
			velSamples
		);
