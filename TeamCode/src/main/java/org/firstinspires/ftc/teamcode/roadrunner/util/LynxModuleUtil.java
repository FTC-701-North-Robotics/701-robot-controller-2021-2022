...
public class LynxModuleUtil {
	...
	private static final LynxFirmwareVersion MIN_VERSION = new LynxFirmwareVersion(
		1,
		8,
		2
	);
	 ...
	 */
	public static class LynxFirmwareVersion
		implements Comparable<LynxFirmwareVersion> {
	...
	public static void ensureMinimumFirmwareVersion(HardwareMap hardwareMap) {
		...
		if (outdatedModules.size() > 0) {
			...
			msgBuilder.append(
				"One or more of the attached Lynx modules has outdated firmware\n"
			);
			msgBuilder.append(
