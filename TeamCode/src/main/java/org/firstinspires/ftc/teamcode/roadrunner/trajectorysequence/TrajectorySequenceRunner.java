...
public class TrajectorySequenceRunner {
	...
	public @Nullable DriveSignal update(
		Pose2d poseEstimate,
		Pose2d poseVelocity
	) {
		...
		if (currentTrajectorySequence != null) {
			...
			if (currentSegment instanceof TrajectorySegment) {
				Trajectory currentTrajectory =
					((TrajectorySegment) currentSegment).getTrajectory();

				if (isNewTransition) follower.followTrajectory(
					currentTrajectory
				);
				...
				MotionState targetState =
					((TurnSegment) currentSegment).getMotionProfile()
						.get(deltaTime);

				turnController.setTargetPosition(targetState.getX());

				double correction = turnController.update(
		...
		if (
			POSE_HISTORY_LIMIT > -1 && poseHistory.size() > POSE_HISTORY_LIMIT
		) {
		...
		packet.put(
			"headingError (deg)",
			Math.toDegrees(getLastPoseError().getHeading())
		);
