...
public class skcusmac extends OpMode {
	...
	private final DcMotor spinny = null;
	private DcMotor winch = null;

	private final DcMotor dropDown = null;

	private final CRServo leftIntake = null;
	private final CRServo rightIntake = null;
		...
		if (gamepad2.right_stick_y < -0.05) {
			...
			if (intakeup == 0) {
				if (intake.getCurrentPosition() >= -90) {
					...
					intake.getCurrentPosition() < -90 &&
					intake.getCurrentPosition() >= -100
				) {
					intake.setPower(-0.4);
